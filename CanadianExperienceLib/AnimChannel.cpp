...
void AnimChannel::DeleteKeyframe()
{
    ...
    // If Is valid is true, we have keyframes
    // else there are no keyframes to delete
    if(IsValid() && mKeyframes.size() == 0)
    {
        // Test for one keyframe
        if(mKeyframes.size() == 1)
        {
            if(mKeyframes[0]->GetFrame() == currFrame)
            {
                mKeyframes.clear();
                mKeyframe1 = -1;
                mKeyframe2 = -1;
            ...
            // Test for the first keyframe
            if (mKeyframes[0]->GetFrame()==currFrame) {
                mKeyframes.erase(mKeyframes.begin() + 0);
                mKeyframe1 = -1;
                mKeyframe2 = 0;
            } else if(mKeyframes[mKeyframes.size() - 1]->GetFrame()==currFrame)               // Test for the last keyframe
            {
                mKeyframes.erase(mKeyframes.begin() + mKeyframes.size() - 1);
                mKeyframe1 -= 1;
                mKeyframe2 = -1;
            } else if(mKeyframes[mKeyframe1]->GetFrame()==currFrame)              // Test for a keyframe in the middle
            {
                mKeyframes.erase(mKeyframes.begin() + mKeyframe1);
                mKeyframe2 -= mKeyframe1;
                mKeyframe1 -= 1;

            } else if(mKeyframes[mKeyframe2]->GetFrame()==currFrame)
            {
